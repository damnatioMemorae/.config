...
ShellRoot {
    ...
    // Theme colors
    property color colSpark: "#add8e6"
    property color colBg: "#0e0e16"
    ...
    Process {
        id: memProc
        command: ["sh", "-c", "free --mega | grep Mem"]
        stdout: SplitParser {
            onRead: data => {
                if (!data) return
                var parts = data.trim().split(/\s+/)
                ...
                RowLayout {
                    ...
                    Repeater {
                        ...
                        Rectangle {
                            ...
                            Text {
                                text: index + 1
                                color: parent.isActive ? root.colSpark : (parent.hasWindows ? root.colPurple : root.colMuted)
