...
local function shortPath(path)
        ...
        local shortened = iter(parts)
            ...
            :map(function(_p, part)
                    return guard {
                            isRaw(part) or _p == 1 or _p == #parts, function()
                            return part
                    end,
...
o.quickfixtextfunc = function(info)
        ...
        local list = match(info.quickfix) {
                [1] = function() return fn.getqflist(what) end,
                _   = function() return fn.getloclist(info.winid, what) end,
        }
        local prep = function(_)
                ...
                local fname   = _.lnum > 0 and match(_.bufnr) {
                        [2] = function() return { "[" .. shortPath(bufname) .. "]:", "Special" } end,
                        _   = function() return { shortPath(bufname) .. ":", type_hilights[_.type] or "QfFilename" } end,
                } or nil
                return {
                        { prefix, hl },
                        fname,
...
local toggle = function(key, h, w)
        ...
        return function()
                local split    = match(key) {
                        [p.lower] = function() return { "", "resize " .. perc "lines" (h or 50) } end,
                        [p.upper] = function() return { "wincmd L", "vertical resize " .. perc "columns" (w or 50) } end,
                }
